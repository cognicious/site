...
openssl aes-256-cbc -d -in .ci/github_deploy_key.enc -out $HOME/github_deploy_key.pub -K $enc_key -iv $enc_iv -d && \
# Set Git config
git config core.sshCommand 'ssh -i $HOME/github_deploy_key.pub' && \
